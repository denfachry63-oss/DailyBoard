export async function ambilKutipan() {
    ...
    const tombolRefresh = document.getElementById("btn-refresh-kutipan");

    elKutipan.style.color = "";
    elKutipan.textContent = "Memuat kutipan...";

    if (tombolRefresh) {
        tombolRefresh.disabled = true;
        tombolRefresh.style.opacity = "0.6";
    }
    ...
    try {
        ...
        if (tombolRefresh) {
            tombolRefresh.disabled = false;
            tombolRefresh.style.opacity = "1";
        }
    }
}
